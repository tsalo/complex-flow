...
def init_func_derivatives_wf(
    ...
    **Parameters**
    ...
    ds_preproc_bold = pe.MapNode(
        DerivativesDataSink(base_directory=output_dir, desc="preproc", suffix="bold"),
    ...
    if set(["func", "run", "bold", "boldref", "sbref"]).intersection(output_spaces):
        ...
        ds_bold_native_ref = pe.Node(
            DerivativesDataSink(
                base_directory=output_dir, suffix="boldref", compress=True
            ),
        ...
        workflow.connect(
            [
                ...
                (
                    inputnode,
                    ds_bold_native_ref,
                    [("source_file", "source_file"), ("bold_native_ref", "in_file")],
                ),
                (
                    inputnode,
                    ds_bold_mask_native,
                    [("source_file", "source_file"), ("bold_mask_native", "in_file")],
                ),
    ...
    if "T1w" in output_spaces or "anat" in output_spaces:
        ...
        ds_bold_t1_ref = pe.Node(
            DerivativesDataSink(
                base_directory=output_dir, space="T1w", suffix="boldref", compress=True
            ),
            name="ds_bold_t1_ref",
            run_without_submitting=True,
            mem_gb=DEFAULT_MEMORY_MIN_GB,
            ...
            workflow.connect(
                [
                    (
                        inputnode,
                        ds_bold_aseg_t1,
                        [("source_file", "source_file"), ("bold_aseg_t1", "in_file")],
                    ),
                    (
                        inputnode,
                        ds_bold_aparc_t1,
                        [("source_file", "source_file"), ("bold_aparc_t1", "in_file")],
                    ),
                ]
            )
            ...
            workflow.connect(
                [
                    ...
                    (
                        inputnode,
                        cifti_bolds,
                        [("bold_cifti", "in_file"), ("source_file", "source_file")],
                    ),
